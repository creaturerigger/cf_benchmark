...
class TestDeduplicator:
    ...
    def test_different_query_id_same_features_preserved(self, deduplicator):
        # Same feature values but different query_ids — NOT a duplicate
        df = pd.DataFrame({'a': [1.0, 1.0], 'b': [2.0, 2.0], 'query_id': ['id1', 'id2']})
        result = deduplicator(df)
        assert len(result) == 2

    def test_same_query_id_same_features_deduped(self, deduplicator):
        df = pd.DataFrame({'a': [1.0, 1.0], 'b': [2.0, 2.0], 'query_id': ['id1', 'id1']})
        result = deduplicator(df)
        assert len(result) == 1
    ...
    def test_no_query_id_column(self, deduplicator):
